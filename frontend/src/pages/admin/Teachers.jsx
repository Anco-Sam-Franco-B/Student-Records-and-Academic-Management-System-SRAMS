...
import { BookOpen, Search, X, Briefcase, Plus, Trash2, LoaderPinwheelIcon } from 'lucide-react';
...
export default function Teachers() {
  ...
  const fetchData = async () => {
    try {
      const [techRes, tradesRes, subRes, classRes, allocRes] = await Promise.all([
  ...
  const handleUpdatePrimaryTrade = async () => {
    ...
    try {
      ...
      fetchData();
      alert("Primary trade updated successfully!");
    } catch (error) {
  ...
  const handleAddAllocation = async (e) => {
    ...
    try {
      ...
      alert(error.response?.data?.error || "Failed to add allocation");
    } finally {
      setSaving(false);
    }
  };
